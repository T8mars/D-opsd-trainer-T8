...
def prepare_batch_image_latents(
    ...
    if images.ndim != 5:
        raise ValueError(f"expected image tensor with 4 or 5 dims, got {images.shape}")

    batch_size, num_images, channels, height, width = images.shape
    reference_images = images.reshape(batch_size * num_images, channels, height, width)
    reference_images = reference_images.to(device=device, dtype=vae_dtype)
    encoded_flat = pipeline._encode_vae_image(image=reference_images, generator=generator)
    latent_channels, latent_height, latent_width = encoded_flat.shape[1:]
    encoded_image_latents = encoded_flat.reshape(
        batch_size,
        num_images,
        latent_channels,
        latent_height,
    ...
    for image_idx in range(num_images):
        image_latents.append(pipeline._pack_latents(encoded_image_latents[:, image_idx]))
        image_id_latents.append(encoded_image_latents[:1, image_idx])

    image_latent_ids = pipeline._prepare_image_ids(image_id_latents)
    image_latent_ids = image_latent_ids.repeat(batch_size, 1, 1).to(device)
    image_latents = torch.cat(image_latents, dim=1).to(device=device, dtype=latent_dtype)
...
def main(args):
    ...
    prompt_keys = ["user_prompt_en", "short_en", "user_prompt_zh", "short_zh"]
    ...
    for epoch in range(epoch_start + 1, args.epochs):
        for batch in train_dataloader:
            ...
            with accelerator.accumulate(gen_model):
                ...
                reference_images = batch["reference_pixel_values"].to(device=accelerator.device, dtype=vae_dtype)
                target_images = batch["target_pixel_values"].to(device=accelerator.device, dtype=vae_dtype)
                train_dtype = inference_dtype
                prompts = batch["prompts"]

                bsz = target_images.shape[0]
                h, w = target_images.shape[2], target_images.shape[3]
                ...
                with torch.no_grad():
                    ...
                    with accelerator.autocast():
                        ...
                        student_image_latents, student_image_latent_ids = prepare_batch_image_latents(
                            pipeline=pipeline,
                        ...
                        teacher_condition_images = torch.cat(
                            [reference_images, target_images.unsqueeze(1)],
                            dim=1,
                        )
                        teacher_image_latents, teacher_image_latent_ids = prepare_batch_image_latents(
                            pipeline=pipeline,
                            images=teacher_condition_images,
                            device=accelerator.device,
                            vae_dtype=vae_dtype,
                            latent_dtype=train_dtype,
                            generator=None,
                ...
                for back_step in range(len(timesteps)):
                    ...
                    with accelerator.autocast():
                        ...
                        student_hidden_states = torch.cat([latents_student, student_image_latents], dim=1)
                        student_img_ids = torch.cat([latent_ids, student_image_latent_ids], dim=1)
                ...
                if accelerator.sync_gradients:
                    ...
                    if args.save_samples and args.sample_steps > 0 and (global_step % args.sample_steps == 0 or global_step == args.max_train_steps):
                        with torch.no_grad():
                            ...
                            gen_model.set_adapter("student")
                            with accelerator.autocast():
                                images_s = run_pipeline_per_edit_sample(
                                    pipeline=pipeline,
                                    prompts=test_prompts,
                                    condition_image_paths=test_reference_image_paths,
                                    height=sample_h,
                            ...
                            with accelerator.autocast():
                                ...
                                teacher_condition_image_paths = [
                                    ref_paths + [target_path]
                                    for ref_paths, target_path in zip(
                                        test_reference_image_paths,
                                        test_target_image_paths,
                                    )
                                ]
                                images_t = run_pipeline_per_edit_sample(
                                    pipeline=pipeline,
