...
_FLOAT_COMPUTE_DTYPES = (torch.bfloat16, torch.float16, torch.float32)


def _grad_input_dtype(weight: torch.Tensor, fallback: torch.dtype) -> torch.dtype:
    if weight.dtype in _FLOAT_COMPUTE_DTYPES:
        return weight.dtype
    return fallback
...
class _BouncingLinearFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        ...
        # grad wrt input (GPU)
        grad_input = grad_out.to(dtype=_grad_input_dtype(w_bwd, target_dtype)) @ w_bwd
...
class _BouncingConv2dFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        ...
        grad_input = conv2d_input(
            x.shape,
            w_bwd,
            grad_out.to(dtype=_grad_input_dtype(w_bwd, target_dtype)),
            stride=stride,
            padding=padding,
            dilation=dilation,
            groups=groups,
