...
- 16GB 显存安全默认值：low VRAM、8-bit Adam、分辨率缩放、tiled VAE、冻结组件 CPU 转移和可选实验性 block offload。
...
更高分辨率、更长步数、FLUX2 9B 或 block offload 组合需要重新评估显存占用。当前推荐配置默认不启用 block offload。
...
- `block offload` 是高级实验选项，UI 默认使用 1 block/group；开启前建议先用默认配置跑通一小段训练。
...
需要注意：ai-toolkit 的 `Layer Offloading` 是训练期 `MemoryManager.attach(..., offload_percent=...)` 风格的按层 CPU/GPU 内存管理，并带有 Transformer/Text Encoder 百分比配置。当前训练器尚未实现这套训练期百分比层卸载；现有 `block offload` 是 Diffusers group block offload，能力和验证范围不同。
