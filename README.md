...
- 新建训练任务时可选择多个数据集合并训练，并为每个数据集设置训练权重。
- 支持自定义样图提示词；一行一个样图提示词，训练集和样图集会分开写入。
- 单 GPU FIFO 任务队列、任务克隆、停止、删除、日志查看、输出目录打开。
- 左侧 TensorBoard 工作台可直接查看 LOSS 曲线、梯度范数、平滑趋势和多运行对比。
- 16GB 显存安全默认值：low VRAM、8-bit Adam、分辨率缩放、tiled VAE、冻结组件 CPU 转移，以及可选实验性 block offload / Layer Offloading。
...
- 删除整个数据集。
- 在新建训练任务中选择多个数据集合并训练。
- 为每个数据集设置训练权重；训练权重为整数 0-10，`0` 可用于临时排除某个已选数据集。
...
2. 选择训练配方：
   ...
   - FLUX2 Klein Editing
   - Z-Image Turbo VLM
3. 选择一个或多个数据集，并为每个数据集设置训练权重。
4. 使用默认的 16GB 推荐配置，或按需调整步数、样图提示词、checkpoint、显存选项。
5. 创建草稿。
6. 在「任务」页启动任务。
7. 训练过程中查看日志、loss、GPU 占用和产物；需要完整曲线时打开左侧「TensorBoard」。
...
更高分辨率、更长步数、FLUX2 9B、block offload 或 Layer Offloading 组合需要重新评估显存占用。当前推荐配置默认不启用这些实验卸载选项。

### LOSS 曲线和 TensorBoard

- 左侧「TensorBoard」页面会自动发现 `trainer-data/runs`、`trainer-data/smoke-runs` 和任务账本中的训练输出。
- 内置曲线默认读取每个运行的 `loss_log/loss_gen_log.jsonl`，支持总 LOSS、D-OPSD LOSS、梯度范数、平滑、对数轴和多运行对比。
- 训练脚本默认同时写出 TensorBoard event 文件，位置通常是：

```text
trainer-data/runs/<exp-name>/tensorboard/events.out.tfevents.*
```

- 如果想用外部 TensorBoard 查看同一个运行，可以在 WSL 训练环境中执行：

```bash
tensorboard --logdir trainer-data/runs/<exp-name>/tensorboard
```

- 若只想保留 JSONL 而不写 event，可在手动启动 smoke 脚本时设置 `TENSORBOARD=0`。

### 训练参数说明

- 「总步数」是主要停止条件，建议优先调整它。
- 底层脚本仍会收到 `EPOCHS`，但它由训练器按 `MAX_TRAIN_STEPS + 1` 自动派生。
- 训练权重为整数 0-10；多个数据集会合并为临时 JSONL，并按权重重复采样行。
- 样图提示词是一行一个样图提示词；留空时使用所选数据集，填写后会生成单独的样图 JSONL，不改变训练 JSONL。
- `low VRAM`、`8-bit Adam`、样图缩放和 tiled VAE 默认适合 16GB 起步配置。
- `block offload` 是最终采样路径的高级实验选项，UI 默认使用 1 block/group。
- `Layer Offloading` 是训练期按层 CPU/GPU 卸载，可分别设置 Transformer 和 Text Encoder 卸载比例；默认关闭，建议先用默认配置跑通一小段训练再尝试。
...
- 冻结组件阶段性转移到 CPU。
- 可选 Diffusers group block offload。当前已验证的是 FLUX2 Identity 延迟最终采样路径，默认 1 block/group；如果该实验路径仍触发 CUDA OOM，最终采样器会回退到 model CPU offload；推荐配置仍不默认开启。
- 可选 ai-toolkit 风格训练期 Layer Offloading，通过 `MemoryManager.attach(..., offload_percent=...)` 对 Transformer/Text Encoder 的 Linear/Conv 层做 CPU/GPU 迁移；UI 可分别设置两个百分比，推荐配置仍默认关闭。

需要注意：`Layer Offloading` 和 `block offload` 不是同一个功能。前者用于训练期按层卸载，后者用于 Diffusers group block offload/最终采样兜底；两者默认都不会改变 16GB 推荐起步配置。
